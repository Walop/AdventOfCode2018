...
let findClosest (gridPoint: Point) (inputPoints: Point seq) =
    let distances =
        getDistances gridPoint inputPoints

    let min =
        distances
        |> Seq.minBy (fun d -> snd d)
        |> snd

    let closestPoints =
        distances
        |> Seq.filter (fun d -> (snd d) = min)
...
let part1 (input: Point seq) =
    ...
    let excluded =
        claims
        |> Seq.filter (fun p -> p.X = boundaries.MinX || p.X = boundaries.MaxX || p.Y = boundaries.MinY || p.Y = boundaries.MaxY)
        |> Seq.map (fun p -> p.Id)
        |> Set.ofSeq
        |> Set.add 99
