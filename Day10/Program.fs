...
type Point =
    struct
        ...
        new(x, y, dx, dy) = { X = x; Y = y; DX = dx; DY = dy }
    end
...
let printPoints (points: Point list) =
    let minX = points |> List.minBy (fun p -> p.X) |> (fun p -> p.X)

    let maxX = points |> List.maxBy (fun p -> p.X) |> (fun p -> p.X)

    let minY = points |> List.minBy (fun p -> p.Y) |> (fun p -> p.Y)

    let maxY = points |> List.maxBy (fun p -> p.Y) |> (fun p -> p.Y)

    let positions = points |> List.map (fun p -> (p.X, p.Y)) |> List.sort
...
let rec part1and2 t input =
    ...
    let allPoints = input |> List.map (fun pat -> pat t)

    let allNeighbours = allPoints |> List.forall (fun p -> hasNeighbour p allPoints)
...
let main argv =
    ...
    input |> part1and2 0 |> printfn "Took %i seconds"
