...
let rec getCarts (carts: Cart seq) x row =
    if Seq.isEmpty row then
        ...
        if List.contains (snd head) [ '<'; '^'; '>'; 'v' ] then
            let carts' =
                Seq.append carts (Seq.singleton (Cart(x, fst head, snd head, turnlist)))

            getCarts carts' x tail
        else
            getCarts carts x tail

let dirMap = Map [ '<', (-1, 0); '^', (0, -1); '>', (1, 0); 'v', (0, 1) ]
...
[<EntryPoint>]
let main argv =
    let inputlines = Util.readLines (__SOURCE_DIRECTORY__ + "\\input")

    let inputwidth = inputlines |> Seq.head |> Seq.length
